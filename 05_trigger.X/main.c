...
unsigned char sum_frame_num = 32; // number of frame, in the display signal cache
...
#define display_clear_4 display_ctrl = 3
#define display_clear_1 display_ctrl = 2
...
void display_cache_push_back(unsigned char D)
{
    if (display_ctrl & 0x02)
    // can only push display data in loop mode
    {
        display_cache[dis_cache_size] = D;
        dis_cache_size++;
    }
}
...
void LED_Show(void)
{
    ...
    {
        display_cache_push_back(digital_decode[i]);
    }
    frame_repeat_num = 10;
    sum_frame_num = 10;
    display_clear_1;
